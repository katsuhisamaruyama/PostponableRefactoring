...
public class ExtractMethodRefactoringP extends ExtractMethodRefactoring {
    ...
    protected ImportRewrite fImportRewriter;
    protected int fSelectionStart;
    protected int fSelectionLength;
    protected AST fAST;
    protected ASTRewrite fRewriter;
    protected ExtractMethodAnalyzerP fAnalyzer;
    protected int fVisibility;
    protected String fMethodName;
    protected boolean fThrowRuntimeExceptions;
    protected List<ParameterInfo> fParameterInfos;
    protected Set<String> fUsedNames;
    protected boolean fGenerateJavadoc;
    protected boolean fReplaceDuplicates;
    protected SnippetFinderP.Match[] fDuplicates;
    protected int fDestinationIndex = 0;
    protected ASTNode fDestination;
    protected ASTNode[] fDestinations;
    protected LinkedProposalModel fLinkedProposalModel;
    ...
    public ExtractMethodRefactoringP(ICompilationUnit unit, int selectionStart, int selectionLength) {
    ...
    public ExtractMethodRefactoringP(JavaRefactoringArguments arguments, RefactoringStatus status) {
        this((ICompilationUnit) null, 0, 0);
        RefactoringStatus initializeStatus = initialize(arguments);
        status.merge(initializeStatus);
    }
    
    public ExtractMethodRefactoringP(CompilationUnit astRoot, int selectionStart, int selectionLength) {
    ...
    private ASTVisitor createVisitor() throws CoreException {
        fAnalyzer = new ExtractMethodAnalyzerP(fCUnit, Selection.createFromStartLength(fSelectionStart, fSelectionLength));
        return fAnalyzer;
    }
    ...
    public RefactoringStatus checkVarargOrder() {
        for (Iterator<ParameterInfo> iter = fParameterInfos.iterator(); iter.hasNext();) {
            ...
            if (info.isOldVarargs() && iter.hasNext()) {
                return RefactoringStatus.createFatalErrorStatus(Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_error_vararg_ordering,
    ...
    private void initializeDuplicates() {
        ...
        fDuplicates = SnippetFinderP.perform(start, fAnalyzer.getSelectedNodes());
    ...
    @SuppressWarnings("unchecked")
    private ASTNode[] createCallNodes(SnippetFinderP.Match duplicate, int modifiers) {
        ...
        switch (returnKind) {
            case ExtractMethodAnalyzerP.ACCESS_TO_LOCAL:
                ...
                break;
            case ExtractMethodAnalyzerP.RETURN_STATEMENT_VALUE:
                ReturnStatement rs = fAST.newReturnStatement();
        ...
        if (returnKind == ExtractMethodAnalyzerP.RETURN_STATEMENT_VOID && !fAnalyzer.isLastStatementSelected()) {
            result.add(fAST.newReturnStatement());
        }
        return result.toArray(new ASTNode[result.size()]);
    }
    
    private IVariableBinding getMappedBinding(SnippetFinderP.Match duplicate, IVariableBinding org) {
        if (duplicate == null) {
            return org;
        }
    ...
    private String getMappedName(SnippetFinderP.Match duplicate, ParameterInfo paramter) {
        if (duplicate == null) {
    ...
    private void replaceDuplicates(CompilationUnitChange result, int modifiers) {
        ...
        for (int d = 0; d < fDuplicates.length; d++) {
            SnippetFinderP.Match duplicate = fDuplicates[d];
    ...
    private boolean forceStatic() {
        ...
        for(int i = 0;i < fDuplicates.length; i++) {
            SnippetFinderP.Match duplicate = fDuplicates[i];
    ...
    public boolean getThrowRuntimeExceptions() {
        return fThrowRuntimeExceptions;
    }
    ...
    protected RefactoringStatus initialize(JavaRefactoringArguments arguments) {
